      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="pkg in packages" :key="pkg.id">
            ...
            <td class="px-6 py-4">
              <span v-if="pkg.deleted_at" class="text-red-600">已刪除</span>
            ...
            <td class="px-6 py-4 text-right space-x-2">
              ...
              <BaseButton v-if="!pkg.deleted_at" size="sm" variant="danger" @click="confirmDelete(pkg)">刪除</BaseButton>
...
import { getPackages, deletePackage, type ServicePackage } from '@/api/modules/servicePackage';
...
const confirmDelete = async (pkg: ServicePackage) => {
  if (confirm(`確定將組合包「${pkg.name}」標記為刪除嗎？`)) {
